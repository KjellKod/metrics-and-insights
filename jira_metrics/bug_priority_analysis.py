...
def parse_arguments():
    ...
    parser.add_argument(
        "--all-projects", action="store_true", help="Analyze ALL projects (ignores JIRA_PROJECTS env var)"
    )
...
def analyze_bug_priorities(tickets):
    ...
    priority_stats = defaultdict(lambda: {"count": 0, "completion_times": [], "tickets": []})
...
def display_results(priority_stats):
    ...
    sorted_priorities = sorted(priority_stats.items(), key=lambda x: x[1]["count"], reverse=True)
    ...
    with open(filename, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for priority, stats in priority_stats.items():
            for ticket_info in stats["tickets"]:
                writer.writerow([ticket_info["key"], priority, ticket_info["completion_time"], ticket_info["summary"]])
