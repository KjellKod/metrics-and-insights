...
def main(argv: list[str] | None = None, github_client: GitHubClient | None = None) -> int:
    ...
    sections = []
    sections.extend(build_jira_sections(jira_summary, config.teams, config.date_window))
    sections.extend(build_github_sections(github_summary, config.date_window))
    sections.append(build_recommendations(jira_summary, github_summary, config.teams, config.date_window))
